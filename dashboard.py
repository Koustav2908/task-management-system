...
st.sidebar.info('Predict task Category & Priority. Logs history, export CSV, Slack alerts on high priority.')

# Main title
st.title('🔮 Insightful Task Category & Priority Predictor')
st.write('Enter a task title and description to see predictions, insights, and history log.')
...
if st.button('Predict'):
    if not title or not desc:
        ...
        # Add to history
        record = {'Timestamp': timestamp, 'Title': title, 'Description': desc, 'Priority': prio_label, 'Category': cat_label}
...
if st.session_state.history:
    ...
    st.download_button(label='📥 Download History as CSV', data=csv_buffer.getvalue(), file_name='prediction_history.csv')
